...
const AllScenarios = () => {
  ...
  const handleDelete = (scenarioId) => {
    ...
    const deleteVehiclesPromises = vehiclesToDelete.map(vehicle => 
      axios.delete(`http://localhost:3001/vehicleDelete/${vehicle.id}`)
    );
    ...
    Promise.all(deleteVehiclesPromises)
      .then(() => {
        ...
        axios
          .delete(`http://localhost:3001/scenarioDelete/${scenarioId}`)
          .then(() => {
  ...
  const handleSaveScenario = () => {
    axios
      .put(
        `http://localhost:3001/scenarioEdit/${selectedScenario}`,
        scenarioForm
      )
      .then((response) => {
        setScenarios(
  ...
  const handleSaveVehicle = () => {
    axios
      .post("http://localhost:3001/vehicles", vehicleForm)
      .then((response) => {
